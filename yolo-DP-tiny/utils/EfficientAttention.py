...
class EfficientAttention(nn.Module):
    ...
    def __init__(self,c1, dim, num_heads=16, group_split=[8,8], kernel_sizes=[7], window_size=0,
                 attn_drop=0., proj_drop=0., qkv_bias=True):
        super().__init__()
        assert sum(group_split) == num_heads
        assert len(kernel_sizes) + 1 == len(group_split)
        if dim==64: window_size=8
        elif dim == 128: window_size = 4
        elif dim == 256:window_size = 2
        elif dim == 512:window_size = 1
    ...
    def low_fre_attention(self, x: torch.Tensor, to_q: nn.Module, to_kv: nn.Module, avgpool: nn.Module):
        ...
        kv = to_kv(kv)
        kv = kv.view(b, 2, -1, self.dim_head, (h * w) // (self.window_size ** 2))
        kv = kv.permute(1, 0, 2, 4, 3).contiguous()  # (2 b m (H W) d)
        k, v = kv  # (b m (H W) d)
