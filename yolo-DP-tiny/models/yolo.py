...
class DetectionModel(BaseModel):
    ...
    def _initialize_dh_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        m = self.model[-1]  # Decoupled_Detect() module
        for mi, s in zip(m.m, m.stride):  # Duyệt qua từng tầng P3, P4, P5

            # 1. Khởi tạo bias cho nhánh Objectness (Vật thể)
            if hasattr(mi, 'obj_preds'):
                b = mi.obj_preds.bias.view(m.na, -1)
                b.data[:, 0] += math.log(8 / (640 / s) ** 2)
                mi.obj_preds.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)

            # 2. Khởi tạo bias cho nhánh Classification (Phân loại)
            if hasattr(mi, 'cls_preds'):
                b = mi.cls_preds.bias.data
                b += math.log(0.6 / (m.nc - 0.999999)) if cf is None else torch.log(cf / cf.sum())
                mi.cls_preds.bias = torch.nn.Parameter(b, requires_grad=True)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                BottleneckCSP, C3,  C3TR, C3SPP, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, Cfc, C3g, EfficientAttention, LSKmodule, PConv,
                LSKAttention,  C2f, C2f_Ghost, C2f_Faster, C2f_LR, BottleneckDS, SPPF_LR, DSConv, LowRankConv1x1, GhostConv_LR, ACBlock, DSConv_AC,
                MobileOneBlock, DSConv_MO, FasterNetBlock, RELAN, UIBLite, SCDown, PConvGhostLite}:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x, C3g,  C2f, C2f_Ghost, C2f_Faster, C2f_LR, RELAN}:
